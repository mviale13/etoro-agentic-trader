...
class ArtificialCIO:
    ...
    def _determine_state(
        ...
        if evidence.risk_score > policy.maximum_acceptable_risk:
            return (
                DecisionState.REJECT,
                "Risk exceeds the maximum permitted by policy.",
            )

        if evidence.quality_score < policy.minimum_watchlist_quality:
            return (
                DecisionState.REJECT,
    ...
    def _calculate_conviction(
        ...
        positive_score = (
            ...
            + (100 - evidence.risk_score)
        ) / 5

        conviction = round(positive_score)
